...
---
module: otc_pool
short_description: Add/Delete a pool in the load balancing service from OpenTelkomCloud
...
options:
   ...
   lb_algorithm:
    ...
    session_persistence:
      description:
        - Persistence configuration for the pool.
      suboptions:
        type:
          description:
        ...
        cookie_name:
          description:
            - Specifies the cookie name for the APP_COOKIE type.
        persistence_timeout:
          description:
...
def _wait_for_pool_status(module, cloud, pool_id, status, failures,
    ...
    while total_sleep < timeout:
        pool = cloud.network.get_pool(pool_id)
        if pool:
            return pool
...
def main():
    argument_spec = openstack_full_argument_spec(
        ...
        session_persistence=dict(default=None, type='dict')
    )
