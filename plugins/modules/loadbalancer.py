    ...
    def run(self):
        ...
        if self.params['state'] == 'present':
            if not lb:
                ...
                lb = self.conn.loadbalancer.create_load_balancer(
            ...
            if floating_ip:
                ...
                lb = self.conn.loadbalancer.find_load_balancer(name_or_id=lb.id)
                lb_dict = lb.to_dict()
            ...
            if lb:
                if self.ansible.check_mode:
                    self.exit_json(changed=True)
                self.conn.loadbalancer.delete_load_balancer(lb, cascade=True)
                changed = True
